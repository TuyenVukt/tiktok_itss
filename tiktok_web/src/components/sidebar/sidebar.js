    ...
    <div className="sidebar">
      <div className="sidebar--div--change--pages">
        {window.location.pathname == "/" ? (
          <Link className="sidebar--for--you key" to="/">
            <HomeIcon
              style={{
                marginRight: 20 + "px",
              }}
            />{" "}
            {props.lang("sidebar.for_you")}
          </Link>
        ) : (
          <Link className="sidebar--for--you" to="/">
            <HomeIcon
              style={{
                marginRight: 20 + "px",
              }}
        ...
        {window.location.pathname == "/following" ? (
          <Link className="sidebar--following key" to="/following">
            <GroupRoundedIcon
              style={{
                marginRight: 20 + "px",
              }}
            />{" "}
            {props.lang("sidebar.following")}
          </Link>
        ) : (
          <Link className="sidebar--following" to="/following">
            <GroupRoundedIcon
              style={{
                marginRight: 20 + "px",
              }}
      ...
      <div className="sidebar--div--suggest">
        <h1 className="sidebar--div--title">
          {props.lang("sidebar.suggest_user")}
        </h1>
        {all.map((val) => {
      ...
      <div className="sidebar--div--following">
        <h1 className="sidebar--div--title">
          {props.lang("sidebar.following_user")}
        </h1>
        {all.map((val) => {
          if (val.id != localStorage.getItem("id")) {
            if (following.includes(val.id)) {
      ...
      <div className="sidebar--div--hot--hashtags">
        <h1 className="sidebar--div--title">
          {props.lang("sidebar.musics_recommend")}
        </h1>
      </div>
    </div>
  );
